...
/**
 * EditPart for the a ExperimentGraph instance.
 * <p>
 ...
 */
class GraphEditPart extends AbstractGraphicalEditPart implements
	...
	private ExperimentGraph getCastedModel() {
		return (ExperimentGraph) getModel();
	}
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		// the ExperimentGraph instance and must cause a call of refreshChildren()
		// to update the diagram's contents.
		if (ExperimentGraph.CHILD_ADDED_PROP.equals(prop)
				|| ExperimentGraph.CHILD_REMOVED_PROP.equals(prop)) {
			refreshChildren();
		}
	}
	...
	private static class ShapesXYLayoutEditPolicy extends XYLayoutEditPolicy {
		...
		protected Command createChangeConstraintCommand(
			...
			if (child instanceof GraphNodeEditPart
				...
				// return a command that can move and/or resize a GraphNode
				return new GraphNodeSetConstraintCommand((GraphNode) child.getModel(),
		...
		protected Command getCreateCommand(CreateRequest request) {
			...
			if (childClass == ProtocolNode.class
					|| childClass == InputNode.class
					|| childClass == OutputNode.class) {
				// return a command that can add a node to a Graph
				return new GraphNodeCreateCommand((GraphNode) request.getNewObject(), (GraphicalViewer)getHost().getViewer(),
						(ExperimentGraph) getHost().getModel(),
